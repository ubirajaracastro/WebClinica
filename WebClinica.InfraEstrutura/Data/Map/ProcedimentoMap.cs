using System.Data.Entity.ModelConfiguration;
using WebClinica.Dominio.Entidades;
using System.ComponentModel.DataAnnotations.Schema;
using System.Data.Entity.Infrastructure.Annotations;

namespace WebClinica.InfraEstrutura.Data.Map
{
    public class ProcedimentoMap : EntityTypeConfiguration<Procedimento>
    {
        public ProcedimentoMap()
        {
            ToTable("TblProcedimento");

            Property(x => x.ProcedimentoId)
           .HasDatabaseGeneratedOption(DatabaseGeneratedOption.Identity);
            HasKey(x => x.ProcedimentoId);
                    

            Property(x => x.Nome)
           .IsRequired();

            Property(x => x.Descricao)
           .IsRequired();

            Property(x => x.TempMedio)
          .IsRequired();

            Property(x => x.Ativo);
            Property(x => x.VincularAProcedimentoTUSS);

            Property(x => x.CNPJClinica)
               .IsRequired()
                .HasColumnAnnotation(
                    IndexAnnotation.AnnotationName,
                    new IndexAnnotation(new IndexAttribute("IX_CNPJ", 1) { IsUnique = false }));

            Property(x => x.CodigoTSUS);

            HasRequired(x => x.TipoProcedimento);
           
        }

    }
}
